...
test("passes AlembicTest non-test direct-thread smoke tasks without a test card", () => {
  const root = makeFixture({
    alembicTestStatus: "待启动",
    alembicTestTask: "非测试型 direct-thread delivery smoke：回填 TargetResultEnvelope 和本窗口投递日志，不运行真实项目测试。",
    testStatus: "已完成",
    planExtra: `

## 目标判断

- 用户目标：direct-thread delivery 真实 smoke。
- 不纳入本轮事项：不运行真实项目测试，不创建 AlembicTest 测试单。

## 测试交接

- 是否需要 \`AlembicTest\`：否；本轮 AlembicTest 只作为非测试型可见窗口参与 direct-thread delivery smoke。
`,
  });
  const result = run(root);
  assert.equal(result.status, 0, result.stderr);
