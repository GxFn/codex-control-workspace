...
function isNonTestTargetTask(row, planContent) {
  ...
  const isAutomationSmoke =
    /Codex Automation Closed Loop|codex automation|自动化|direct-thread|delivery envelope|TargetResultEnvelope|smoke|冒烟/i.test(
      planSignals,
    ) && /smoke|冒烟|direct-thread|delivery|自动化|可见窗口|TargetResultEnvelope|回填/i.test(row.task);
