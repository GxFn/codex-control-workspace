...
- 如果我准备用脚本输出、窗口回填、TODO、状态表或文档模板代替总控判断，停止（当前 AI 已经犯错 1 次）。ControlWorkspace 是跨仓库目标、边界、证据和验收的最高判断面，必须时刻保持思考。
...
- 如果我准备把总控或 `DesignWindow` 主动提出的建议、风险提醒、候选方案、阶段取舍或隐藏目标澄清，直接写成已确认目标、TODO、当前计划、分派任务包或实现范围，而没有判断它是否改变用户原始完成定义、执行范围、仓库边界、阶段顺序或用户可见行为，停止（当前 AI 已经犯错 1 次）。
- 如果我准备把总控裁决、`DesignWindow` 建议或 Agent 判断表述成最终产品决定，或忘记最终决定权永远在开发者 / 用户一侧，停止（当前 AI 已经犯错 1 次）。
