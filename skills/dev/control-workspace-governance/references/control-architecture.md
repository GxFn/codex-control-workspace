...
| Closed-loop controller return | Target result envelopes for a dispatch group are ready or a target reports blocked. | Controller-return direct thread delivery plus `review-results`; unattended mode immediately continues review / next-wave dispatch when allowed. | Acceptance decision, next-wave dispatch, hard stop gate, missing evidence, final completion, user stop, or no eligible TODO. |
